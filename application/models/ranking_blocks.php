...
class ranking_blocks extends \Fyre\Core\Model {

    public $table_name  = "ranking_blocks"; 
    public $schema      = array (
                            "id",
                            "name",
                            "title",
    ...
    */
    ...
    function listing($lang) {
        ...
        $sql = "SELECT * FROM " . $this->table_name . " WHERE lang = :lang AND active = 1 ORDER BY name DESC";
        $query = $this->db->prepare($sql);
        $query->execute(array(":lang" => $lang));
        return $query->fetchAll();
    }
