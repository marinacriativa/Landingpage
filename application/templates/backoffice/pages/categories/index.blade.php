    ...
    <div class="p-desktop-5">
        <div class="mb-3">
            <div class="d-flex justify-content-between">
                <h1 class="inline">{{ ucfirst($translations["backoffice"]["categories"]) }}</h1>
                <button id="orderASC" type="button" class="btn btn-lg btn-primary" >Ordenar A-Z</button>
            </div>
            <hr>
            <div id="categories" class="container card p-4 general_items">
...
function categories() {
    ...
    function categoriesInit() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                /* $(".parent-element").sortable({
                    containment: "document",
                    items: ".child-element",
                    placeholder: "item-placeholder",
                    connectWith: '.parent-element'
                }); */
    ...
    function populateCategories(categories) {
        ...
        $.each(categories, function(key, category) {
            ...
            let category_template = $(`
                <li data-id="` + category.id + `" class=' all-scroll ` + ((category.root == "1") ? "mb-5" : "pl-5") + ` ${element}'>
    ...
    function categoriesListners() {
        ...
        // Ao clicar no botão orderChildrens
        $("#orderASC").off("click");
            ...
            buttons: {
                formSubmit: {
                    ...
                    action: function () {
                        ...
                        $.ajax({
                            url: "/api/categories/" + id + "?name=" + name,
                            type: "PUT",
                            dataType: "json",
                            success: function (response) {
    ...
    function orderASCCategory() {

        $.confirm({
            title: "{{ ucfirst($translations["backoffice"]["confirm"]) }}",
            theme: "supervan",
            content: 'Deseja ordenar todas as categorias em ordem alfabética?',
            buttons: {

                Sim: function () {
                    $.ajax({
                        url: "/api/categories/asc",
                        type: "POST",
                        dataType: "json",
                        success: function (response) {
