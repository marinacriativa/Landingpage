    ...
    <main class="default-transition pt-5 mb-5 collapsable-main no-load">
        <div class="p-desktop-5">
            <div class="row">
                <div class="col-12">
                    <div class="mb-3">
                        <h1>{{ ucfirst($translations["backoffice"]["title_list_products"]) }}</h1>
                        ...
                        <nav class="breadcrumb-container d-none d-sm-block d-lg-inline-block" aria-label="breadcrumb">
                            <ol class="breadcrumb pt-0">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">
                                    {{ ucfirst($translations["backoffice"]["products"]) }}
                                </li>
                            </ol>
                        </nav>
                    </div>
...
{{--                                    class="simple-icon-arrow-down align-middle"></i></a>--}}
                        <div class="collapse d-block" id="displayOptions">
                            <div class="d-block d-md-inline-block">
                                ...
                                @endif
                                <div class="btn-group float-md-left mr-1 mb-1">
                                    <button class="btn btn-outline-dark btn-xs dropdown-toggle products-order-menu"
                                            type="button" data-toggle="dropdown" aria-haspopup="true"
                                            aria-expanded="false">{{ ucfirst($translations["backoffice"]["order_by"]) }}
                                    </button>
                                    <div class="dropdown-menu">
                                        <a class="dropdown-item order-menu" data-order="created_at">{{ ucfirst($translations["backoffice"]["orderBy_last_inserted"]) }}</a>
                                ...
                                <div class="btn-group float-md-left mr-1 mb-1">
                                    <button class="btn btn-outline-dark btn-xs dropdown-toggle products-category-menu"
                                            type="button" data-toggle="dropdown" aria-haspopup="true"
                                            aria-expanded="false">Categoria
                                    </button>
                                    <div class="dropdown-menu" id="listCategories">                                   
...
@section('scripts')
    <script>
        ...
        function load() {
            ...
            window.history.replaceState("", "", '/adm/products?' + $.param( { limit: limit, order: order, page: page, search: search, lang:lang, category:category } ));

            $.ajax({
                url: "/api/products?page=" + page + "&search=" + search + "&length=" + limit + "&order=" + order + "&lang=" + lang + "&category=" + category,
                type: "GET",
                dataType: "json",
                success: function (response) {
                    ...
                    populate(response.data['0']);    

                    document.querySelector('#listCategories').innerHTML = `<a class="dropdown-item category-menu" data-id="false">Todos</a>`;     
                    $.each(response.data['2'], function (key, item) {     
                        document.querySelector('#listCategories').innerHTML += `
                            <a class="dropdown-item category-menu" data-id="`+ item.id +`">`+ item.name +`</a>
                        `;
                    })
                    ...
                    if (response.data['1'] !== undefined) {
                        ...
                        let pagination  = response.data['1'];
            ...
            <div class="card d-flex flex-row mb-3" data-id="` + item.id + `">
                ...
                <div class="pl-2 d-flex flex-grow-1 min-width-zero">
                    <div class="card-body align-self-center d-flex flex-column flex-lg-row justify-content-between min-width-zero align-items-lg-center">
                        ...
                        <p class="mb-0 text-muted text-small w-10 w-sm-100 m-1">` + item.sku + `</p>
                        <p class="mb-0 text-muted text-small w-10 w-sm-100 m-1">` + item.price + ` €</p>
                        <p class="mb-0 text-muted text-small w-10 w-sm-100 m-1">` + item.previous_price + ` €</p>
                        <p class="mb-0 text-muted text-small w-10 w-sm-100 m-1">` + item.stock + ` uni.</p>
                        <div class="w-15 w-sm-100 m-1">
                            ...
                            <div class="dropdown-menu dropdown-menu-status-index" data-ticketNumber="` + item.id + `" aria-labelledby="btnGroupDrop1" x-placement="bottom-start" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(97px, 37px, 0px);">
                                <p class="dropdown-item p-0 pl-4" onclick="changeStatus(0, ` + item.id +`)"><span class="dot red mr-2"></span> {{ ucfirst($translations["backoffice"]["products_status_draft"]) }}</p>
                                <p class="dropdown-item p-0 pl-4" onclick="changeStatus(1, ` + item.id +`)"><span class="dot orange mr-2"></span> {{ ucfirst($translations["backoffice"]["products_status_private"]) }}</p>
                                <p class="dropdown-item p-0 pl-4" onclick="changeStatus(2, ` + item.id +`)"><span class="dot green mr-2"></span> {{ ucfirst($translations["backoffice"]["products_status_published"]) }}</p>
                            </div>
                        </div>
                        <div class="w-30 w-sm-100 m-1">
							...
							<button class="btn btn-outline-danger mb-1 btn-xs m-1 float-right" name="delte_slide_btn " title="apagar" id="delte_slide_btn" onclick="removeProduct(` + item.id + `)"><i class="simple-icon-trash"></i></button>
                            <a class="btn btn-outline-secondary mb-1 btn-xs m-1 float-right" name="edit_slide_btn" title="editar" id="edit_slide_btn" href="/adm/products/` + item.id + `"><i class="simple-icon-pencil"></i></a>
                            <a class="btn btn-outline-dark mb-1 btn-xs m-1 float-right" title="copiar" onclick="cloneProduct(` + item.id + `)" data-id="` + item.id + `"><i class="simple-icon-docs"></i></a>
                        </div>
                    </div>
                </div>
            </div>
        ...
        function changeStatus (status, id_product) {            
            $.ajax({
                url: "/api/productsChangeStatus",
                type: "POST",
                data: {id: id_product, status: status},
                dataType: "json",
                success: function (response) {
        ...
        function listeners() {
            ...
            $("body").off("click", ".category-menu");
            $("body").on("click", ".category-menu", function () {              
                if($(this).data("id") == false) {
                    category = '';
                } else {                
                    category = $(this).data("id");
                }
                $('.products-category-menu').html($(this).text())
                load();
            })
            ...
            $(".order-menu").on("click", function () {
                ...
                $(".products-order-menu").text($(this).text());
                load();
            })
