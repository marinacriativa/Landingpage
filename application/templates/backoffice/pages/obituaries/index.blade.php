    ...
    <main class="default-transition pt-5 mb-5 collapsable-main no-load">
        <div class="p-desktop-5">
            <div class="row">
                <div class="col-12">
                    <div class="mb-3">
                        <h1>{{ ucfirst($translations["backoffice"]["obituaries"]) }}</h1>
                        ...
                        <nav class="breadcrumb-container d-none d-sm-block d-lg-inline-block" aria-label="breadcrumb">
                            <ol class="breadcrumb pt-0">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">
                                    {{ ucfirst($translations["backoffice"]["obituaries"]) }}
                                </li>
                            </ol>
                        </nav>
                    </div>
...
{{--                                    class="simple-icon-arrow-down align-middle"></i></a>--}}
                        <div class="collapse d-md-block" id="displayOptions">
                            <div class="d-block d-md-inline-block">
                                ...
                                @endif
                                <div class="btn-group float-md-left mr-1 mb-1">
                                    <button class="btn btn-outline-dark btn-xs dropdown-toggle obituary-order-menu"
                                            type="button" data-toggle="dropdown" aria-haspopup="true"
                            ...
                            <div class="float-md-right">
                                ...
                                <button class="btn btn-outline-dark btn-xs dropdown-toggle obituary-lenght-menu"
...
@section('scripts')
    <script>
        ...
        if (query.limit !== undefined && query.limit.length > 0) {
            ...
            $(".obituary-lenght-menu").text(limit);
        }
        ...
        function load() {
            ...
            window.history.replaceState("", "", '/adm/obituaries?' + $.param( { limit: limit, order: order, page: page, search: search, lang:lang } ));

            $.ajax({
                url: "/api/obituaries?page=" + page + "&search=" + search + "&length=" + limit + "&order=" + order + "&lang=" + lang,
                type: "GET",
                dataType: "json",
        ...
        function create(data) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    window.location.href = "/adm/obituaries/" + response.data.id;
                },
                error: function (jqXHR, textStatus, errorThrown) {
                    $.alert({
                        title: '{{ ucfirst($translations["backoffice"]["error"]) }}',
        ...
        function populate(items) {
            ...
            // <img onerror="this.onerror=null;this.src='/static/backoffice/images/placeholder.png'" src="` + item.photo + `" class="list-thumbnail responsive border-0 card-img-left index-card-img" />
                ...
                <a class="d-flex a-index-img" href="/adm/obituaries/` + item.id + `">
                    <div class="background-image" style="background-image: url(`+ item.photo +`)"></div>
                </a>
                <div class="pl-2 d-flex flex-grow-1 min-width-zero">
                    <div class="card-body align-self-center d-flex flex-column flex-lg-row justify-content-between min-width-zero align-items-lg-center">
                        <p class="mb-0 text-muted text-small w-5 w-sm-100 m-1">` + item.id + `</p>
                        <a href="/adm/obituaries/` + item.id + `" class="w-40 w-sm-100 truncate">
                        ...
                        <div class="w-30 w-sm-100 m-1">
							...
							<button class="btn btn-outline-danger mb-1 btn-xs m-1 float-right" name="delte_slide_btn " title="apagar" id="delte_slide_btn" onclick="removeObituary(` + item.id + `)"><i class="simple-icon-trash"></i></button>
                            <a class="btn btn-outline-secondary mb-1 btn-xs m-1 float-right" name="edit_slide_btn" title="editar" id="edit_slide_btn" href="/adm/obituaries/` + item.id + `"><i class="simple-icon-pencil"></i></a>
                            <a class="btn btn-outline-dark mb-1 btn-xs m-1 float-right" title="copiar" onclick="cloneObituary(` + item.id + `)" data-id="` + item.id + `"><i class="simple-icon-docs"></i></a>
                        </div>
                    </div>
                </div>
            </div>`;
            ...
            if (items === undefined || items.length == 0) {
                ...
                $(".list").html(`
                	<h4>{{ ucfirst($translations["backoffice"]["not_found_male"]) }} {{ $translations["backoffice"]["obituaries"] }}</h4>
        ...
        function changeStatus (status, id_obituary) {            
            $.ajax({
                url: "/api/obituariesChangeStatus",
                type: "POST",
                data: {id: id_obituary, status: status},
                dataType: "json",
                success: function (response) {
                    ...
                    Sim: function () {
                        $.ajax({
                            ...
                            success: function (response) {
                                ...
                                // Rederecionar para a página de index dos obituários
                    ...
                    Sim: function () {
                        $.ajax({
                            ...
                            success: function (response) {
                                ...
                                // Redirecionar para a página de index dos óbitos
                                window.location.replace("/adm/obituaries/");
                            },
                            error: function (jqXHR, textStatus, errorThrown) {
        ...
        function listeners() {
            ...
            $(".order-menu").on("click", function () {
                ...
                $(".obituary-order-menu").text($(this).text());
                load();
            })
