    ...
    <main class="default-transition pt-5 mb-5 collapsable-main no-load">
        <div class="p-desktop-5">
            <div class="row">
                <div class="col-12">
                    <div class="mb-3">
                        <h1>{{ ucfirst($translations["backoffice"]["title_alerts"]) }}</h1>
                        ...
                        <nav class="breadcrumb-container d-none d-sm-block d-lg-inline-block" aria-label="breadcrumb">
                            <ol class="breadcrumb pt-0">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">
                                    {{ ucfirst($translations["backoffice"]["title_alerts"]) }}                                    
                                </li>
                            </ol>
                        </nav>
                    </div>
    ...
    <script>          
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                window.location.href = "/adm/alerts/" + response.data.id;
            },
        ...
        function load() {
            ...
            window.history.replaceState("", "", '/adm/alerts?' + $.param( { limit: limit, order: order, page: page, search: search } ));

            $.ajax({
                url: "/api/alerts?page=" + page + "&search=" + search + "&length=" + limit + "&order=" + order,
                type: "GET",
                dataType: "json",
                success: function (response) {
                    if (!response.success) {
        ...
        function populate(items){
            ...
            <div class="card d-flex flex-row mb-3">
                <a class="d-flex a-index-img" href="/adm/alerts/` + item.id + `">
                    <div class="background-image" style="background-image: url(`+ item.url +`)"></div>
                </a>
                <div class="pl-2 d-flex flex-grow-1 min-width-zero">
                    <div class="card-body align-self-center d-flex flex-column flex-lg-row justify-content-between min-width-zero align-items-lg-center">
                        <a href="/adm/alerts/` + item.id + `" class="w-40 w-sm-100 truncate">
                        ...
                        <div class="w-40 w-sm-100 m-1">
							...
							<button class="btn btn-outline-danger mb-1 btn-xs m-1 float-right" name="delte_slide_btn " title="apagar" id="delte_slide_btn" onclick="removeAlert(` + item.id + `)"><i class="simple-icon-trash"></i></button>
                            <a class="btn btn-outline-secondary mb-1 btn-xs m-1 float-right" name="edit_slide_btn" title="editar" id="edit_slide_btn" href="/adm/alerts/` + item.id + `"><i class="simple-icon-pencil"></i></a>
                            <a class="btn btn-outline-dark mb-1 btn-xs m-1 float-right" title="copiar" onclick="cloneAlert(` + item.id + `)" data-id="` + item.id + `"><i class="simple-icon-docs"></i></a>
                        </div>
                    </div>
                </div>
            </div>
            ...
            if(items === undefined || items.length == 0){
                $(".list").html(`
                    <h4>{{ ucfirst($translations["backoffice"]["not_found_male"]) }} {{ $translations["backoffice"]["title_alerts"] }}</h4>
        ...
        function changeStatus (status, id_alert) {            
            $.ajax({
                url: "/api/alertsChangeStatus",
                type: "POST",
                data: {id: id_alert, status: status},
                dataType: "json",
                success: function (response) {
