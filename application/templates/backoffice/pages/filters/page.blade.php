	...
	<div class="p-desktop-5">
		<div class="row">
			<div class="col-md-12 mb-8">
				<div class="col-12">
					<h1>{{ ucfirst($translations["backoffice"]["title_edit_filters"]) }}</h1>
                    
					<nav class="breadcrumb-container d-none d-sm-block d-lg-inline-block" aria-label="breadcrumb">
						<ol class="breadcrumb pt-0">
							<li class="breadcrumb-item"><a href="/adm">{{ ucfirst($translations["backoffice"]["store"]) }}</a></li>
							<li class="breadcrumb-item"><a href="/adm/filters">{{ ucfirst($translations["backoffice"]["filters"]) }}</a></li>
							<li class="breadcrumb-item active" aria-current="page">{{ ucfirst($translations["backoffice"]["title_edit_filters"]) }}</li>
						</ol>
					</nav>
                    <div class="btn-group top-right-button-container">
                        <button type="button" href="javascript:void(0)" onclick='javascript:window.location.assign("/adm/filters");' class="btn btn-outline-secondary">{{ ucfirst($translations["backoffice"]["edit_custom_btn_back"]) }}</button>
                        @php 
                            $countLangs = 0;
                        @endphp
							...
							<div class="col-md-12 col-xl-8 col-sm-12 col-xs-12 mb-3">
								<div class="card">
									<div class="card-body">
										...
										<hr>
                                        <div class="row">                                         
                                            <div class="form-group mt-4 col-6"><label class="form-group has-float-label"> <input name="name" id="name_x" type="text" autocomplete="off" class="form-control"> <span>{{ ucfirst($translations["backoffice"]["filters_title"]) }}</span> </label> </div>
                                            @if($countLangs > 1)                                                   
                                                <div class="form-group mt-4 col-3">                                                    
                                                    <select name="lang" class="form-control">
                                                        @foreach ($languages as $langs)
                                                            <option class="options_langs" value="{{$langs->code}}" selected="false">{{$langs->name}}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            @endif 
								...
								<div class="card">
									<div class="col-12">
										<div class="card-body">
                                            ...
                                            <button type="button" data-action="save-filter" class="btn btn-outline-info btn-xs mt-2 mr-2">{{ ucfirst($translations["backoffice"]["edit_product_btn_save"]) }}</button> 
...
@section('scripts')
    ...
    function addCategory(filter_id) {
        ...
        $.confirm({
            ...
            title: "{{ ucfirst($translations["backoffice"]["categories_create_title"]) }}",
            theme: "supervan",
            content: '' +
            '<form action="" class="formName">' +
            '<div class="form-group">' +
            '<label>{{ ucfirst($translations["backoffice"]["categories_fill_name"]) }}</label>' +
            '<input type="text" placeholder="{{ ucfirst($translations["backoffice"]["categories_placeholder_name"]) }}" class="name form-control" required />' +
            '</div>' +
            '</form>',
            buttons: {
                formSubmit: {
                    text: '{{ ucfirst($translations["backoffice"]["categories_btn_save"]) }}',
                    action: function () {
                        ...
                        if (!name) {
                            ...
                            $.alert({
                                title: '{{ ucfirst($translations["backoffice"]["error"]) }}',
                                theme: "supervan",
                                content: '{{ ucfirst($translations["backoffice"]["error_message_categories_fill_name_invalid"]) }}',
                            });
    ...
    function deleteCategory(id) {
        ...
        $.confirm({
            title: "{{ ucfirst($translations["backoffice"]["confirm"]) }}",
            theme: "supervan",
            content: '{{ ucfirst($translations["backoffice"]["confirm_message_category_remove"]) }}',
            buttons: {
    ...
    function editCategory(id, old_name) {
        ...
        $.confirm({
            ...
            title: "{{ ucfirst($translations["backoffice"]["categories_edit_title"]) }}",
            theme: "supervan",
            content: '' +
            '<form action="" class="formName">' +
            '<div class="form-group">' +
            '<label>{{ ucfirst($translations["backoffice"]["categories_fill_name"]) }}</label>' +
            '<input type="text" placeholder="{{ ucfirst($translations["backoffice"]["categories_placeholder_name"]) }}" value="' + old_name + '" class="name form-control" required />' +
            '</div>' +
            '</form>',
            buttons: {
                formSubmit: {
