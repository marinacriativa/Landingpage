...
@section('scripts')
    <script>
        ...
        function load() {
            ...
            window.history.replaceState("", "", '/adm/popups?' + $.param( { limit: limit, order: order, page: page, search: search, lang:lang } ));

            $.ajax({
                url: "/api/popups?page=" + page + "&search=" + search + "&length=" + limit + "&order=" + order + "&lang=" + lang,
                type: "GET",
                dataType: "json",
        ...
        function create(data) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    window.location.href = "/adm/popups/" + response.data.data;
                },
                error: function (jqXHR, textStatus, errorThrown) {
                    $.alert({
                        title: '{{ ucfirst($translations["backoffice"]["error"]) }}',
        ...
        function populate(items) {
            ...
            // <img onerror="this.onerror=null;this.src='/static/backoffice/images/placeholder.png'" src="` + item.photo + `" class="list-thumbnail responsive border-0 card-img-left index-card-img" />
                ...
                <a class="d-flex a-index-img" href="/adm/popups/` + item.id + `">
                    <div class="background-image" style="background-image: url(`+ item.photo +`)"></div>
                </a>
                <div class="pl-2 d-flex flex-grow-1 min-width-zero">
                    <div class="card-body align-self-center d-flex flex-column flex-lg-row justify-content-between min-width-zero align-items-lg-center">
                        <a href="/adm/popups/` + item.id + `" class="w-40 w-sm-100 truncate">
                        ...
                        <div class="w-30 w-sm-100 m-1">
							...
							<button class="btn btn-outline-danger mb-1 btn-xs m-1 float-right" name="delte_slide_btn " title="apagar" id="delte_slide_btn" onclick="removePopup(` + item.id + `)"><i class="simple-icon-trash"></i></button>
                            <a class="btn btn-outline-secondary mb-1 btn-xs m-1 float-right" name="edit_slide_btn" title="editar" id="edit_slide_btn" href="/adm/popups/` + item.id + `"><i class="simple-icon-pencil"></i></a>
                            <a class="btn btn-outline-dark mb-1 btn-xs m-1 float-right" title="copiar" onclick="clonePopup(` + item.id + `)" data-id="` + item.id + `"><i class="simple-icon-docs"></i></a>
                        </div>
                    </div>
                </div>
            </div>`;
                    ...
                    Sim: function () {
                        $.ajax({
                            ...
                            success: function (response) {
                                ...
                                window.location.replace("/adm/popups/");
                            },
                            error: function (jqXHR, textStatus, errorThrown) {
                    ...
                    Sim: function () {
                        $.ajax({
                            ...
                            success: function (response) {
                                ...
                                // Redirecionar para a página de index das construções
                                window.location.replace("/adm/popups/");
                            },
                            error: function (jqXHR, textStatus, errorThrown) {
