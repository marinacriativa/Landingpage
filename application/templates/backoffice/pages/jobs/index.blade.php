...
<main class="default-transition pt-5 mb-5 collapsable-main no-load">
	<div class="p-desktop-5">
		<div class="row">
			<div class="col-12">
				<div class="mb-3">
					<h1>{{ ucfirst($translations["backoffice"]["jobs"]) }}</h1>
					...
					<div class="text-zero top-right-button-container">
						<div class="btn-group">
							<a type="button" class="btn btn-primary top-center-button mr-1" data-action="create" href="javascript:void(0)" style="padding: 9px 34px;">{{ ucfirst($translations["backoffice"]["list_job_create"]) }}</a>
						</div>
					</div>				
					<nav class="breadcrumb-container d-none d-sm-block d-lg-inline-block" aria-label="breadcrumb">
						<ol class="breadcrumb pt-0">
							...
							<li class="breadcrumb-item active" aria-current="page">
								{{ ucfirst($translations["backoffice"]["jobs"]) }}
							</li>
						</ol>
					</nav>
				</div>
...
<script>
	...
	*/
	...
	function load() {
		...
		window.history.replaceState("", "", '/adm/jobs?' + $.param( { limit: limit, order: order, page: page, search: search, lang:lang } ));
    
	    $.ajax({
	        url: "/api/jobs?page=" + page + "&search=" + search + "&length=" + limit + "&order=" + order + "&lang=" + lang,
	        type: "GET",
	        dataType : "json",
							...
							<div class="w-20 w-sm-100">
                                <button id="` + item.id +`" data-idTicket ="` + item.id +`"  class="badge badge-pill btn dropdown-toggle bg-primary" data-toggle="dropdown" aria-expanded="false">` + status + `</button>
                                    <div class="dropdown-menu dropdown-menu-status-index" data-ticketNumber="` + item.id + `" aria-labelledby="btnGroupDrop1" x-placement="bottom-start" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(97px, 37px, 0px);">
                                        <p class="dropdown-item" onclick="changeStatus(0, ` + item.id +`)"><span class="dot orange mr-2"></span> {{ ucfirst($translations["backoffice"]["products_status_private"]) }}</p>
                                        <p class="dropdown-item" onclick="changeStatus(1, ` + item.id +`)"><span class="dot green mr-2"></span> {{ ucfirst($translations["backoffice"]["products_status_published"]) }}</p>
                                    </div>
                                </div>
								<p class="mb-0 text-muted text-small w-20 w-sm-100">` + item.location + `</p>
                                <div class="w-40 w-sm-100 ">
									...
									<a class="btn btn-outline-secondary mb-1 btn-xs m-1 float-right" name="edit_slide_btn" title="editar" id="edit_slide_btn" href="/adm/jobs/` + item.id + `"><i class="simple-icon-pencil"></i></a>
	...
	function create() {
		...
		$.ajax({
	        ...
	        success: function (response) {
				...
				window.location.href = "/adm/jobs/" + response.data.id;
	        },
	        error: function(jqXHR, textStatus, errorThrown) {
