                ...
                <div class="col-12">
                    <div class="mb-3">
                        <div class="btn-group top-right-button-container">                           
                            ...
                            @endif
                            @if($countLangs > 1)
                                <button id="btnGroupDrop1" type="button" class="btn btn-secondary dropdown-toggle dropdown-menu-status-index language-selected" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">@if(isset($_GET['langName'])) {{$_GET['langName']}} @else {{$selected_language->name}} @endif</button>
                                    ...
                                    <table class="table table-hover borderless">
                                        <tbody class="languages-list">                                            
                                            @foreach ($languages as $langs)
                                                @if($langs->active == 1)
                                                    <tr>
                                                        <td><a href="?lang={{$langs->code}}&langName={{$langs->name}}" class="select-lang">{{$langs->name}}</a></td>                                                       
                                                    </tr>
                                                @endif
                                            @endforeach
                                        </tbody>
                        ...
                        <h1>{{ ucfirst($translations["backoffice"]["menu_manager"]) }}</h1>
                        ...
                        <nav class="breadcrumb-container d-none d-sm-block d-lg-inline-block" aria-label="breadcrumb">
                            <ol class="breadcrumb pt-0">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">
                                    {{ ucfirst($translations["backoffice"]["menu_manager"]) }}                                    
                                </li>
                            </ol>
                        </nav>
                    </div>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-body pt-0">
                            <form>    
                                ...
                                <div class="form-group mt-4">
                                    <label class="form-group has-float-label"><input placeholder="{{ ucfirst($translations["backoffice"]["menus_placeholder_icon"]) }}" name="icon" class="form-control"><span>{{ ucfirst($translations["backoffice"]["icon_menu"]) }}</span></label>     
                                </div>
                                <div class="form-group">
                                    <label class="form-group has-float-label"><input placeholder="{{ ucfirst($translations["backoffice"]["menus_fill_name"]) }}" name="name" class="form-control" autocomplete="off"><span>{{ ucfirst($translations["backoffice"]["menus_fill_name"]) }}</span></label>     
                                </div>
                                <div class="form-group" id="searchitems">
                                    <label class="form-group has-float-label"><input placeholder="{{ ucfirst($translations["backoffice"]["menus_fill_name"]) }}" name="search-items" class="form-control" autocomplete="off"><span>Pesquisar</span></label>     
                                    <div id="resultsSearch">                                    
                                    </div>  
                                </div>
                                <div class="form-group" id="url_input">
                                    <label class="form-group has-float-label"><input placeholder="{{ ucfirst($translations["backoffice"]["url_fill_name"]) }}" name="url" class="form-control"><span>{{ ucfirst($translations["backoffice"]["url_fill_name"]) }}</span></label> 
                                </div>
                                <fieldset class="form-group">
                                    <div class="row">
                                        <label class="col-form-label col-lg-12 pt-0">{{ ucfirst($translations["backoffice"]["newtab"]) }}</label>
                                        <div class="col-lg-12">
                                            <div class="form-check">
                                                <input class="form-check-input" type="radio" name="newtab" id="newtab-false" value="false" checked>
                                                <label class="form-check-label" for="newtab-false">{{ ucfirst($translations["backoffice"]["menu_newtab_false"]) }}</label>
                                            </div>
                                            <div class="form-check">
                                                <input class="form-check-input" type="radio" name="newtab" id="newtab-true" value="true">
                                                <label class="form-check-label" for="newtab-true">{{ ucfirst($translations["backoffice"]["menu_newtab_true"]) }}</label>
                                            </div>
                                        </div>
                                    </div>
                                </fieldset>    
                                ...
                                <input type="hidden" name="lang" value="@if(isset($_GET['lang'])){{$_GET['lang']}}@else{{$selected_language->code}}@endif">                        
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="button" class="btn btn-danger remove-menu btn-sm">{{ ucfirst($translations["backoffice"]["banner_btn_remove"]) }}</button>
                            <button type="button" class="btn btn-primary save-menu btn-sm">{{ ucfirst($translations["backoffice"]["banner_btn_save"]) }}</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>       
       ...
       function menu_manager() {
            ...
            function populateMenus(menus, count = 0) {
                ...
                let startId = 'class="p-0 nested-sortable"';
                if(count == 0) {                 
                    startId = 'class="p-0 nested-sortable" id="menuManagerSortable"';
                } 
                let template = $("<ul " + startId + "></ul>");
