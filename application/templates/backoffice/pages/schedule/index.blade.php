...
<main class="default-transition pt-5 mb-5 collapsable-main no-load">
	<div class="p-desktop-5">
		<div class="row">
			<div class="col-12">
				<div class="mb-3">
					<h1>{{ ucfirst($translations["backoffice"]["schedule"]) }}</h1>
					...
					<div class="text-zero top-right-button-container">
						<div class="btn-group">
							<a type="button" class="btn btn-primary top-center-button mr-1" data-action="create" href="javascript:void(0)" style="padding: 9px 34px;">{{ ucfirst($translations["backoffice"]["list_schedule_create"]) }}</a>
						</div>
					</div>				
					<nav class="breadcrumb-container d-none d-sm-block d-lg-inline-block" aria-label="breadcrumb">
						<ol class="breadcrumb pt-0">
							...
							<li class="breadcrumb-item active" aria-current="page">
								{{ ucfirst($translations["backoffice"]["schedule"]) }}
							</li>
						</ol>
					</nav>
				</div>
...
<script>
	...
	*/
	...
	function load() {
		...
		window.history.replaceState("", "", '/adm/schedule?' + $.param( { limit: limit, order: order, page: page, search: search, lang:lang } ));
    
	    $.ajax({
	        url: "/api/schedule?page=" + page + "&search=" + search + "&length=" + limit + "&order=" + order + "&lang=" + lang,
	        type: "GET",
	        dataType : "json",
		...
		$.each(items, function(key, item) {
			...
			let template = `
			    <div class="card d-flex flex-row mb-3 all-scroll" data-id="` + item.id + `">                   
                    <div class="pl-2 d-flex flex-grow-1 min-width-zero" data-id="` + item.id + `">
                        <div class="card-body align-self-center d-flex flex-column flex-lg-row justify-content-between min-width-zero align-items-lg-center">
                            <a href="/adm/schedule/` + item.id + `" class="w-60 w-sm-100 truncate">
							...
							<div class="w-20 w-sm-100">
                                ...
                                <div class="w-40 w-sm-100 ">
									...
									<button class="btn btn-outline-danger mb-1 btn-xs m-1 float-right" name="delte_slide_btn " title="apagar" id="delte_slide_btn" onclick="removeSchedule(` + item.id + `)"><i class="simple-icon-trash"></i></button>
									<a class="btn btn-outline-secondary mb-1 btn-xs m-1 float-right" name="edit_slide_btn" title="editar" id="edit_slide_btn" href="/adm/schedule/` + item.id + `"><i class="simple-icon-pencil"></i></a>
									<a class="btn btn-outline-dark mb-1 btn-xs m-1 float-right" title="copiar" onclick="cloneSchedule(` + item.id + `)" data-id="` + item.id + `"><i class="simple-icon-docs"></i></a>
                                </div>
                            </div>
                        </div>
                    </div>`;
	...
	function create() {
		...
		$.ajax({
	        ...
	        success: function (response) {
				...
				window.location.href = "/adm/schedule/" + response.data.id;
	        },
	        error: function(jqXHR, textStatus, errorThrown) {
