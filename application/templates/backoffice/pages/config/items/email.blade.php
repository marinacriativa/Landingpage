<div id="email_settings">
    <h4 class="inline">{{ ucfirst($translations["backoffice"]["email_title"]) }}</h4>
    <hr>
    <label>{{ ucfirst($translations["backoffice"]["email_logo"]) }}</label>
    <div id="email-image"></div>
    <br><br>

    <label class="form-group has-float-label">
        <input class="form-control" type="text" name="smtp_host" data-config-name="smtp_host" placeholder="">
        <span>{{ ucfirst($translations["backoffice"]["email_host"]) }}</span>
    </label>

    <label class="form-group has-float-label">
        <input class="form-control" type="text" name="smtp_port" data-config-name="smtp_port" placeholder="">
        <span>{{ ucfirst($translations["backoffice"]["email_port"]) }}</span>
    </label>

    <label class="form-group has-float-label">
        <input class="form-control" type="text" name="smtp_username" data-config-name="smtp_username" placeholder="">
        <span>{{ ucfirst($translations["backoffice"]["email_username"]) }}</span>
    </label>

    <label class="form-group has-float-label">
        <input class="form-control" type="text" name="smtp_password" data-config-name="smtp_password" placeholder="">
        <span>{{ ucfirst($translations["backoffice"]["email_password"]) }}</span>
    </label>

    <label class="form-group has-float-label">
        <input class="form-control" type="text" name="smtp_security" data-config-name="smtp_security" placeholder="">
        <span>{{ ucfirst($translations["backoffice"]["email_security"]) }}</span>
    </label>

    <button type="button" class="btn btn-outline-primary btn-xs float-right ml-2" data-settings="smtp_host,smtp_port,smtp_username,smtp_password,smtp_security">{{ ucfirst($translations["backoffice"]["email_btn_save"]) }}</button>
    <button type="button" class="btn btn-outline-primary btn-xs float-right test-smtp">{{ ucfirst($translations["backoffice"]["email_btn_test"]) }}</button>
</div>
...
<script>
    ...
    function email() {
        ...
        $("#email_settings [data-settings]").on("click", function() {
            ...
            $.alert({
                ...
                theme: "supervan",
                content: '{{ ucfirst($translations["backoffice"]["success_message_save_settings_email"]) }}',
            });
        });
    ...
    function test() {
        ...
        $.confirm({
            ...
            '<label>{{ ucfirst($translations["backoffice"]["email_fill_email"]) }}</label>' +
            '<input type="email" placeholder="{{ ucfirst($translations["backoffice"]["email_placeholder_email"]) }}" class="name form-control" required />' +
            '</div>' +
            '</form>',
            buttons: {
                formSubmit: {
                    ...
                    action: function () {
                        ...
                        if (!email) {
                            ...
                            window.notification.display("{{ ucfirst($translations["backoffice"]["email_fill_email_invalid"]) }}");
                            return false;
                        }
