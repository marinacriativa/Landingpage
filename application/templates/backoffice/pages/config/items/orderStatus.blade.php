<h4 class="w-100">{{ ucfirst($translations["backoffice"]["orderStatus_title"]) }}
    <div class="btn-group top-right-button-container">
        <a href="#" class="btn btn-xs btn-outline-secondary add-new-status">{{ ucfirst($translations["backoffice"]["orderStatus_btn_create"]) }}</a>
        @php 
            $countLangs = 0;
        @endphp
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
                    ...
                    <form>
                        <div class="row">
                            <div class="form-group col mb-6">
                                <label>{{ ucfirst($translations["backoffice"]["orderStatus_fill_name"]) }}</label>
                                <input name="name" type="text" autocomplete="off" class="form-control">
                            </div>
                            <div class="form-group col mb-6">
                                <label>{{ ucfirst($translations["backoffice"]["orderStatus_fill_language"]) }}</label>
                                <select name="lang" class="form-control">
                                </select>
                            </div>
                        </div>
                        <label class="form-group ckeditor has-float-label">{{ ucfirst($translations["backoffice"]["orderStatus_fill_email_content"]) }}
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-light btn-sm" data-dismiss="modal">{{ ucfirst($translations["backoffice"]["orderStatus_btn_cancel"]) }}</button>
                <button type="button" class="btn btn-primary btn-sm save-status" data-id="">{{ ucfirst($translations["backoffice"]["orderStatus_btn_save"]) }}</button>
            </div>
        </div>
    </div>
</div>
    ...
    function orderStatus() {
                    ...
                    `);
                        ...
                        <div class="tab-pane fade ` + ((language.default === "1") ? "active show" : "") +
                            ...
                            <table class="dt-responsive stripe languages-table dataTable no-footer dtr-inline" id="thetable3` + language.code + `" role="grid" style="width: 0px;">
                                <thead class="thead">
                                    <tr>
                                        <th class="sorting_asc" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" style="width: 0px;" aria-sort="ascending" aria-label="Nome: activate to sort column descending">{{ ucfirst($translations["backoffice"]["orderStatus_fill_name"]) }}</th>
        ...
        function removeStatus(id) {
            ...
            $.confirm({
                ...
                theme: "supervan",
                content: '{{ ucfirst($translations["backoffice"]["confirm_message_remove_orderStatus"]) }}',
                buttons: {
