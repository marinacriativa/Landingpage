<h4 class="w-100">{{ ucfirst($translations["backoffice"]["shipping_title"]) }}
    <div class="btn-group top-right-button-container">
        <a href="#" class="btn btn-xs btn-outline-secondary add-new-shipping">{{ ucfirst($translations["backoffice"]["shipping_btn_create"]) }}</a>
        @php 
            $countLangs = 0;
        @endphp
    ...
    <div class="modal-dialog" role="document" style="max-width: 90%;">
        <div class="modal-content">
                ...
                <div class="col-md-6 mx-auto">
                    <form>
                        <div class="form-group has-float-label col mb-4">
                            <label class="ml-3">{{ ucfirst($translations["backoffice"]["shipping_fill_name"]) }}</label>
                            <input name="title" type="text" autocomplete="off" class="form-control">
                        </div>
                        <div class="form-group has-float-label col mb-4">
                            <label class="ml-3">{{ ucfirst($translations["backoffice"]["shipping_fill_description"]) }}</label>
                            <input name="subtitle" type="text" autocomplete="off" class="form-control">
                        </div>
                        <div class="form-group has-float-label col mb-4">
                            <label class="ml-3">{{ ucfirst($translations["backoffice"]["shipping_fill_price"]) }}</label>
                            <input name="price" type="number" min="0" autocomplete="off" class="form-control">
                        </div>
                        
                        <div class="form-group has-float-label col mb-4">
                            <label class="ml-3">{{ ucfirst($translations["backoffice"]["shipping_fill_language"]) }}</label>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-light btn-sm" data-dismiss="modal">{{ ucfirst($translations["backoffice"]["shipping_btn_cancel"]) }}</button>
                <button type="button" class="btn btn-primary btn-sm save-shipping-method" data-id="">{{ ucfirst($translations["backoffice"]["shipping_btn_save"]) }}</button>
            </div>
        </div>
    </div>
</div>
                        ...
                        <div class="tab-pane fade ` + ((language.default === "1") ? "active show" : "") +
                            ...
                            <div class="table-responsive">
                                <table class="dt-responsive stripe languages-table dataTable no-footer dtr-inline thetable" id="thetable` + language.code + `" role="grid" style="width: 0px;">
                                    <thead class="thead">
                                        <tr>
                                            <th class="sorting_asc" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" style="width: 0px;" aria-sort="ascending" aria-label="Nome: activate to sort column descending">{{ ucfirst($translations["backoffice"]["shipping_fill_name"]) }}</th>
                                            <th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" style="width: 0px;" aria-label="Código: activate to sort column ascending">{{ ucfirst($translations["backoffice"]["shipping_fill_description"]) }}</th>
                                            <th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" style="width: 0px;" aria-label="Autor: activate to sort column ascending">{{ ucfirst($translations["backoffice"]["shipping_fill_price"]) }}</th>
...
let valorid = 1; 
        ...
        function listnersShipping() {
            ...
            $(".add-new-shipping").on("click", function() {
                ...
                $("#shipping-editor .modal-title").text("{{ ucfirst($translations["backoffice"]["shipping_create_title"]) }}");
            ...
            $(".edit-shipping").on("click", function() {
                ...
                // Alterar o titulo do modal
                $("#shipping-editor .modal-title").text("{{ ucfirst($translations["backoffice"]["shipping_edit_title"]) }}");
        ...
        function removeShipping(id) {
            ...
            $.confirm({
                ...
                theme: "supervan",
                content: '{{ ucfirst($translations["backoffice"]["confirm_message_remove_shipping"]) }}',
                buttons: {
