    ...
    <main class="default-transition pt-5 mb-5 collapsable-main no-load">
        <div class="p-desktop-5">
            <div class="row">
                <div class="col-12">
                    <div class="mb-3">
                        <h1>{{ ucfirst($translations["backoffice"]["title_cycles_blocks"]) }}</h1>
                        ...
                        <nav class="breadcrumb-container d-none d-sm-block d-lg-inline-block" aria-label="breadcrumb">
                            <ol class="breadcrumb pt-0">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">
                                    {{ ucfirst($translations["backoffice"]["title_cycles_blocks"]) }}                                    
                                </li>
                            </ol>
                        </nav>
                    </div>
    ...
    <script>          
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                window.location.href = "/adm/cycles_blocks/" + response.data.id;
            },
        ...
        function load() {
            ...
            window.history.replaceState("", "", '/adm/cycles_blocks?' + $.param( { lang: lang } ));

            $.ajax({
                url: "/api/cycles_blocks?lang=" + lang,
                type: "GET",
                dataType: "json",
                success: function (response) {
                    if (!response.success) {
                            ...
                            if ($("#big-banners-languages-list").find(".nav-item").length < window.LANGUAGES.length) {
                                ...
                                $("#big-banners-languages-list").append(`
                                    <li class="nav-item w-100 bg-white p-0">
                                        ...
                                        `" href="/adm/cycles_blocks?lang=` + language.code +`">` + language.name + `</a>  
                                    </li>
                                `);            
        ...
        function populate(items){
                    ...
                    <div class="card d-flex flex-row mb-3">                
                        <div class="pl-2 d-flex flex-grow-1 min-width-zero">
                            <div class="card-body align-self-center d-flex flex-column flex-lg-row justify-content-between min-width-zero align-items-lg-center">
                                <a href="/adm/cycles_blocks/` + item.id + `" class="w-40 w-sm-100 truncate">
                                ...
                                <div class="w-40 w-sm-100 m-1">		
                                    ...
                                    <a class="btn btn-outline-secondary mb-1 btn-xs m-1 float-right" name="edit_slide_btn" title="editar" id="edit_slide_btn" href="/adm/cycles_blocks/` + item.id + `"><i class="simple-icon-pencil"></i></a>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            if(items === undefined || items.length == 0){
                $(".list").html(`
                    <h4>{{ ucfirst($translations["backoffice"]["not_found_male"]) }} {{ $translations["backoffice"]["title_cycles_blocks"] }}</h4>
                    ...
                    Sim: function () {
                        $.ajax({
                            ...
                            success: function (response) {
                                ...
                                window.location.replace("/adm/cycles_blocks/");
                            },
                            error: function (jqXHR, textStatus, errorThrown) {
