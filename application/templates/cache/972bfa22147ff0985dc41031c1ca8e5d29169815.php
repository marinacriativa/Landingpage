    ...
    <main class="default-transition pt-5 mb-5 collapsable-main no-load">
        <div class="p-desktop-5">
            <div class="row">
                <div class="col-12">
                    <div class="mb-3">
                        <h1><?php echo e(ucfirst($translations["backoffice"]["title_list_galleries"])); ?></h1>
                        ...
                        <nav class="breadcrumb-container d-none d-sm-block d-lg-inline-block" aria-label="breadcrumb">
                            <ol class="breadcrumb pt-0">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">
                                    <?php echo e(ucfirst($translations["backoffice"]["galleries"])); ?>
...
<?php $__env->startSection('scripts'); ?>
    <script>
        ...
        function load() {
            ...
            window.history.replaceState("", "", '/adm/galleries?' + $.param( { limit: limit, order: order, page: page, search: search } ));

            $.ajax({
                url: "/api/galleries?page=" + page + "&search=" + search + "&length=" + limit + "&order=" + order,
                type: "GET",
                dataType: "json",
        ...
        function create(data) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    window.location.href = "/adm/galleries/" + response.data.id;
                },
                error: function (jqXHR, textStatus, errorThrown) {
                    $.alert({
                        title: '<?php echo e(ucfirst($translations["backoffice"]["error"])); ?>',
        ...
        function populate(items) {
            ...
            // <img onerror="this.onerror=null;this.src='/static/backoffice/images/placeholder.png'" src="` + item.photo + `" class="list-thumbnail responsive border-0 card-img-left index-card-img" />
                ...
                let template = `
                    <div class="card d-flex flex-row mb-3 all-scroll">
                        <a class="d-flex a-index-img" href="/adm/galleries/` + item.id + `">
                            <div class="background-image" style="background-image: url(`+ item.photo +`)"></div>
                        </a>
                        <div class="pl-2 d-flex flex-grow-1 min-width-zero">
                            <div class="card-body align-self-center d-flex flex-column flex-lg-row justify-content-between min-width-zero align-items-lg-center">
                                <a href="/adm/galleries/` + item.id + `" class="w-60 w-sm-100 truncate">
                                ...
                                <div class="w-40 w-sm-100 ">
									...
									<button class="btn btn-outline-danger mb-1 btn-xs m-1 float-right" name="delte_slide_btn " title="apagar" id="delte_slide_btn" onclick="removeGallery(` + item.id + `)"><i class="simple-icon-trash"></i></button>
                                    <a class="btn btn-outline-secondary mb-1 btn-xs m-1 float-right" name="edit_slide_btn" title="editar" id="edit_slide_btn" href="/adm/galleries/` + item.id + `"><i class="simple-icon-pencil"></i></a>
                                    <a class="btn btn-outline-dark mb-1 btn-xs m-1 float-right" title="copiar" onclick="cloneGallery(` + item.id + `)" data-id="` + item.id + `"><i class="simple-icon-docs"></i></a>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            if (items === undefined || items.length == 0) {
                ...
                $(".list").html(`
                	<h4><?php echo e(ucfirst($translations["backoffice"]["not_found_male"])); ?> <?php echo e($translations["backoffice"]["galleries"]); ?></h4>
        ...
        function changeStatus (status, id_gallery) {            
            $.ajax({
                url: "/api/galleriesChangeStatus",
                type: "POST",
                data: {id: id_gallery, status: status},
                dataType: "json",
                success: function (response) {
                    ...
                    // Redirecionar para a página de index das galerias
                    load()
                },
                error: function (jqXHR, textStatus, errorThrown) {
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\wamp64\www\CRIATIVA\application\templates\backoffice/pages/galleries/index.blade.php ENDPATH**/ ?>